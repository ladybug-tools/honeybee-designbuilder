...
class ModelDesignBuilderProperties(object):
    """DesignBuilder Properties for Honeybee Model.
    ...
    def __init__(self, host):
        """Initialize ModelDesignBuilderProperties."""
        self._host = host
    ...
    def check_for_extension(self, raise_exception=True, detailed=False):
        """Check that the Model is valid for DesignBuilder simulation.

        This process includes all relevant honeybee-core checks as well as checks
        that apply only for DesignBuilder.
        ...
        # perform checks that are specific to DesignBuilder
    ...
    def to_dict(self):
        """Return Model DesignBuilder properties as a dictionary."""
        return {'designbuilder': {'type': 'ModelDesignBuilderProperties'}}
    ...
    def apply_properties_from_dict(self, data):
        ...
        """
        assert 'designbuilder' in data['properties'], \
            'Dictionary possesses no ModelDesignBuilderProperties.'
        room_dsb_dicts = []
        if 'rooms' in data and data['rooms'] is not None:
            for room_dict in data['rooms']:
                try:
                    room_dsb_dicts.append(room_dict['properties']['designbuilder'])
                except KeyError:
                    room_dsb_dicts.append(None)
            for room, r_dict in zip(self.host.rooms, room_dsb_dicts):
                if r_dict is not None:
                    room.properties.designbuilder.apply_properties_from_dict(r_dict)

    def ToString(self):
        return self.__repr__()

    def __repr__(self):
        return 'Model DesignBuilder Properties: [host: {}]'.format(self.host.display_name)
